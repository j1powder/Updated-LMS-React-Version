            ...
            if(thisUser.id === user.uid) {
                thisUser.courses.map(async(course)=>{
                    if(course.title === 'Abrasive Blasting Safety' && course.score === ""){
                        await updateDocument(user.uid, {courses: arrayRemove({title:"Abrasive Blasting Safety", score:"", passed:"", isAssigned: true})})
                        await updateDocument(user.uid, {courses: arrayUnion({title:"Abrasive Blasting Safety", score:finalScore, passed:"", isAssigned: true})})
