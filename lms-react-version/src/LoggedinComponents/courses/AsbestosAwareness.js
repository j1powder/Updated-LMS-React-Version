...
const AsbestosAwareness = (props) => {
    ...
    const { documents, error } = useCollection('newcourses/Asbestos Awareness Basic/Sections')
       ...
       usersCollection.map((thisUser)=>{
           if(thisUser.id === user.uid) {
               thisUser.courses.map((course)=>{
                   if(course.title === 'Asbestos Awareness Basic' && course.title !== ""){
        ...
        usersCollection.map((thisUser)=>{
            if(thisUser.id === user.uid) {
                thisUser.courses.map(async(course)=>{
                    if(course.title === 'Asbestos Awareness Basic' && course.score === ""){
                        const date = new Date();
                        const todaysDate = {
                            month: 'long',
                            year: 'numeric',
                        ...
                        await updateDocument(user.uid, {courses: arrayRemove({title:"Asbestos Awareness Basic", score:"", date:"", passed:"", isAssigned: true})})
                        await updateDocument(user.uid, {courses: arrayUnion({title:"Asbestos Awareness Basic", score:finalScore, date: date.toLocaleString('en-US', todaysDate), passed:"", isAssigned: true})})
...
const openModal = (e) => {
    e.preventDefault()
    setShow(true);
    const final = document.getElementById('asbestosawareness')
