            ...
            if(thisUser.id === user.uid) {
                thisUser.courses.map(async(course)=>{
                    if(course.title === 'Anhydrous Ammonia' && course.score === ""){
                        ...
                        await updateDocument(user.uid, {courses: arrayRemove({title:"Anhydrous Ammonia", score:"", date:"", passed:"", isAssigned: true})})
                        await updateDocument(user.uid, {courses: arrayUnion({title:"Anhydrous Ammonia", score:finalScore, date: date.toLocaleString('en-US', todaysDate), passed:"", isAssigned: true})})
...
const openModal = (e) => {
    e.preventDefault()
    setShow(true);
    const final = document.getElementById('ammonia')
